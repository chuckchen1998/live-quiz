...
class TestDedup:
    ...
    def test_same_user_same_question_dup(self):
        ...
        assert stats.record_vote("u1", "A") is True
        assert stats.record_vote("u1", "B") is False  # 同用户同题
        assert stats.current.votes["A"] == 1
        assert stats.current.votes["B"] == 0
        assert stats.current.total_votes == 1
...
class TestQuizEngine:
    ...
    def engine(self):
        e = QuizEngine()
        e.questions = [
            type("Q", (), {"id": 1, "question": "Q1", "options": ["A", "B", "C"], "answer": "B"})(),
            type("Q", (), {"id": 2, "question": "Q2", "options": ["A", "B", "C"], "answer": "A"})(),
        ]
        return e
...
class TestIntegration:
    ...
    async def test_full_pipeline_with_replay(self, tmp_path):
        """完整 pipeline: registry → engine → collect → verify"""
        ...
        source = CommentSourceFactory.create("replay", file_path=str(f), speed=50.0)
        engine = QuizEngine()
        engine.questions = [
            type("Q", (), {"id": 1, "question": "Q1", "options": ["A", "B", "C"], "answer": "B"})(),
        ]
...
class TestConcurrency:
    ...
    async def test_concurrent_dedup(self):
        ...
        async def spam(uid: str):
            for _ in range(50):
                stats.record_vote(uid, "A")
                await asyncio.sleep(0)

        tasks = [
            asyncio.create_task(spam("u1")),
            asyncio.create_task(spam("u2")),
        ]
        await asyncio.gather(*tasks)
        assert stats.current.total_votes == 2
