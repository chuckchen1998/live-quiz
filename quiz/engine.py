...
class QuizEngine:
    ...
    async def _emit(self, **kwargs):
        if self._on_state:
            try: await self._on_state(QuizState(question=self.current_question, total_questions=len(self.questions), current_index=self._current_index, **kwargs))
    ...
    async def run(self):
        ...
        for i, q in enumerate(self.questions):
            ...
            await self._emit(phase="result", time_left=config.RESULT_DISPLAY, message=f"正确答案: {q.answer} ({q.points}分)")
            await self._run_timer(config.RESULT_DISPLAY)
            if i < len(self.questions)-1:
                self._current_phase = "between"; self._next_event.clear()
                await self._emit(phase="between", time_left=config.BETWEEN_QUESTIONS, message="下一题即将开始...")
                try: await asyncio.wait_for(self._next_event.wait(), timeout=config.BETWEEN_QUESTIONS)
    ...
    async def _on_tick(self, remaining: int):
        ...
        if self._on_state:
            await self._on_state(QuizState(question=self.current_question, time_left=remaining, phase=self._current_phase, total_questions=len(self.questions), current_index=self._current_index))
