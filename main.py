...
async def main():
    ...
    source_kwargs = {"correct_rate":config.CORRECT_RATE,"interval":config.COMMENT_INTERVAL} if config.COMMENT_SOURCE=="simulator" else {"room_id":config.DOUYIN_ROOM_ID,"cookie":config.DOUYIN_COOKIE} if config.COMMENT_SOURCE=="douyin" else {}
    ...
    async def on_quiz_state(state: QuizState):
        ...
        payload=stats.to_dict(); payload["phase"]=state.phase; payload["time_left"]=state.time_left; payload["message"]=state.message
    ...
    async def comment_loop():
        while True:
            ...
            if event is None: await asyncio.sleep(0.05); continue
            if not (event.answer and stats.current and stats.current.phase=="answering"): continue
